...
//     val pathBuilder = List.newBuilder[(Tree, Int)]
//     var parent = from
...
//     val path0 = pathBuilder.result().reverse
//     println(path0.map { case (t, i) => (shortName(t), i)})

//     new Focus(root = from, path = path0)
//   }
