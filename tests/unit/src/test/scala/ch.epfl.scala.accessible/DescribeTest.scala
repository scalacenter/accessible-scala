...
object DescribeTests extends DescribeTestsUtils {
...

  {
    ...
    check[For]             (q"for (xs <- xss; s <- xs; z = s; if z > 0) f" , "for xs in xss, s in xs, z = s, if z > 0 do f")
    check[ForYield]        (q"for ( x <- xs ) yield x"                     , "for x in xs yield x")
...

  {
    ...
    check[Class] (q"class A { def this(a: Int) { this() } }"     , "class A secondary constructor a Int call to primary constructor")
    check[Class] (
      q"""class A(b: Long) { def this(a: Int) { this(0L); g() } }""", 
      "class A b Long secondary constructor a Int primary constructor applied to 0L , call to g"
    )
    check[Def]   (q"def f = 1"                                   , "def f body: 1")
    check[Def]   (q"def f: Int = 1"                              , "def f returns: Int body: 1")
...

  {
    ...
    check[Source]    ("class A".parse[Source].get  , "class A")
  }
}
